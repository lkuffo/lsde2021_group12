data = [
    {
        "period": "2018-01",
        "topics": [
            {
                "name": "Trump Rally",
                "attention": 1284,
                "likes": 1293,
                "dislikes": 9,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": -0.29,
                "liwc_sentiment": {
                    "anger": 21.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Topic 2",
                "attention": 634,
                "likes": 636,
                "dislikes": 2,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": -0.39,
                "liwc_sentiment": {
                    "anger": 25.18,
                    "posemo": 11.28,
                    "negemo": 10.14,
                    "money": 5.48
                }
            }
        ] 
    },
    {
        "period": "2018-02",
        "topics": [
            {
                "name": "Topic 3",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-03",
        "topics": [
            {
                "name": "Topic 4",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-05",
        "topics": [
            {
                "name": "Topic 5",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-04",
        "topics": [
            {
                "name": "Topic 6",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-06",
        "topics": [
            {
                "name": "Topic 7",
                "attention": 2381,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-07",
        "topics": [
            {
                "name": "Topic 9",
                "attention": 1281,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-08",
        "topics": [
            {
                "name": "Topic 3",
                "attention": 981,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-09",
        "topics": [
            {
                "name": "Topic 10",
                "attention": 928,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-10",
        "topics": [
            {
                "name": "Topic 11",
                "attention": 781,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-11",
        "topics": [
            {
                "name": "Activist Detained and Tortured",
                "attention": 2381,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2018-12",
        "topics": [
            {
                "name": "Other Topic",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-01",
        "topics": [
            {
                "name": "Topic 12",
                "attention": 1281,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-02",
        "topics": [
            {
                "name": "Topic 7",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-03",
        "topics": [
            {
                "name": "Topic 5",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-04",
        "topics": [
            {
                "name": "Topic 5",
                "attention": 1881,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-05",
        "topics": [
            {
                "name": "Topic 11",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    },
    ,
    {
        "period": "2019-06",
        "topics": [
            {
                "name": "Activist Detained and Tortured",
                "attention": 2481,
                "likes": 2490,
                "dislikes": 11,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.39,
                "liwc_sentiment": {
                    "anger": 40.18,
                    "posemo": 18.28,
                    "negemo": 13.14,
                    "money": 10.48
                }
            },
            {
                "name": "Trump Rally",
                "attention": 238,
                "likes": 400,
                "dislikes": 162,
                "keywords": {
                    "Trump": 10, "America": 8, "This": 7,
                    "That": 5, "Other": 4, "No": 4, "Yes": 3,
                    "Anger":2, "Sadness":2, "Emotion": 2
                },
                "sample": [
                    "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
                    "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis",
                    "At vero eos et accusamus et iusto odio dignissimos",
                    "NO!!"
                ],
                "users": [
                    "trump", "quaquity", "auronplay", "biden", "lol"
                ],
                "sentiment": 0.09,
                "liwc_sentiment": {
                    "anger": 8.18,
                    "posemo": 3.28,
                    "negemo": 1.14,
                    "money": 0.48
                }
            }
        ] 
    }
]

// $.getJSON('./output.json', function(data) {         
//     var data = JSON.parse(text);
//     console.log(data);
// });

Highcharts.setOptions({
    chart: {
        style: {
            fontFamily: 'Roboto'
        }
    }
});


let categories = [];
let topicsSeriesMap = {};
let allTopics = [];

data.forEach((period) => {
    categories.push(period.period);
    period.topics.slice(0, 5).forEach((topic) => { // Only top 5 for each period
        if (!(topic.name in topicsSeriesMap)){
            topicsSeriesMap[topic.name] = {
                name: topic.name,
                data: [],
                metadata: {}
            }
            allTopics.push(topic.name);
        }
    })    
})

data.forEach((period) => {
    let topics = period.topics.slice(0, 5); // Only top 5 for each period
    let topicsOnPeriod = [];
    topics.forEach((topic) => {
        topicsSeriesMap[topic.name].data.push(topic.attention);
        topicsSeriesMap[topic.name].metadata[period.period] = {...topic};
        topicsOnPeriod.push(topic.name);
    })
    allTopics.forEach(topic => {
        if (!topicsOnPeriod.includes(topic)){
            topicsSeriesMap[topic].data.push(0.0);
        }
    })
});


console.log(topicsSeriesMap);
console.log(Object.values(topicsSeriesMap));
console.log(categories);

Highcharts.chart('container', {
    chart: {
      type: 'column'
    },
    title: {
      text: 'What is GAB talking about?'
    },
    subtitle: {
      text: 'Analyzing Gab topics & sentiments through time'
    },
    xAxis: {
      categories: categories,
      crosshair: true
    },
    yAxis: {
      min: 0,
      title: {
        text: 'Topic Score (likes - dislikes)'
      }
    },
    tooltip: {
        formatter: function() {
            return  `
                <table style="font-size:12px">
                <span style="font-size:11px">${this.x} topics score</span>
                    ${this.points.filter(point => point.y > 0).map((point) => { return `
                        <tr>
                            <td style="color:${point.color};padding:0">${point.series.name}: </td>
                            <td style="padding:0"><b>${point.y}</b>
                            </td>
                        </tr>                
                    `}).join(' ')
                    }
                </table>
            `
        },
      shared: true,
      useHTML: true
    },
    plotOptions: {
      column: {
        pointPadding: 0.2,
        borderWidth: 0
      },
      series: {
        stacking: 'normal',
        cursor: 'pointer',
        point: {
            events: {
                click: function () {
                    let topicMetadata = topicsSeriesMap[this.series.name].metadata[this.category];
                    let liwc_sentiment = Object.entries(topicMetadata.liwc_sentiment);
                    let vaderSentiment = topicMetadata.vader_sentiment;
                    let sentimentIcon = vaderSentiment < 0.20 ? 'sentiment_very_dissatisfied' : (vaderSentiment > 0.20) ? 'sentiment_satisfied_alt' : 'sentiment_neutral';
                    let sentimentText = vaderSentiment < 0.20 ? 'Overall Negative' : (vaderSentiment > 0.20) ? 'Overall Positive' : 'Overall Neutral';
                    $('.topic-container').append(
                        `
                            <div class="topic-detail animate__animated animate__fadeIn animate__faster">
                                <div class="close-topic">
                                    <span class="material-icons">
                                        highlight_off
                                    </span>
                                </div>
                                <div class="topic-name">
                                    <span style="font-size:11px; font-weight:normal;"> ${this.category} </span>
                                    <div> ${this.series.name} </div>
                                </div>
                                    <div class="topic-content">
                                        <div class="topic-sentiments">
                                            <div class="likes animate__animated animate__fadeIn animate__faster" style="animation-delay: 0.1s;">  
                                                <span class="material-icons">
                                                    thumb_up_off_alt
                                                </span>
                                                <span class="n-likes">
                                                    ${topicMetadata.likes}
                                                </span>
                                            </div>
                                            <div class="likes animate__animated animate__fadeIn animate__faster" style="animation-delay: 0.2s;">    
                                                <span class="material-icons">
                                                    thumb_down_off_alt
                                                </span>
                                                <span class="n-likes">
                                                    ${topicMetadata.dislikes}
                                                </span>
                                            </div>

                                            <span style="font-size:11px; margin-top: 8px; font-weight:normal; color: white; animation-delay: 0.2s;" class="animate__animated animate__fadeIn animate__faster"> 
                                                Sentiments analysis
                                            </span>
                                            <div class="likes animate__animated animate__fadeIn animate__faster" style="animation-delay: 0.3s;">   
                                                <div class="emotions-icons"> 
                                                    <span class="material-icons">
                                                        ${sentimentIcon}
                                                    </span>
                                                    <span class="material-icons">
                                                        ${sentimentIcon}
                                                    </span>   
                                                    <span class="material-icons">
                                                        ${sentimentIcon}
                                                    </span>                                                
                                                </div>

                                                <div class="emotions animate__animated animate__fadeIn animate__faster" style="animation-delay: 0.4s;"> 
                                                    <span class="n-likes" style="font-size: 22px;">
                                                        ${liwc_sentiment[0][0]}: ${Math.round(liwc_sentiment[0][1], 1)}%
                                                    </span>
                                                    <span class="n-likes" style="font-size: 20px;">
                                                        ${liwc_sentiment[1][0]}: ${Math.round(liwc_sentiment[1][1], 1)}%
                                                    </span>
                                                    <span class="n-likes" style="font-size: 18px;">
                                                        ${liwc_sentiment[2][0]}: ${Math.round(liwc_sentiment[2][1], 1)}%
                                                    </span>
                                                </div>
                                            </div>
                                        
                                        </div>
                                        
                                        <div id="topic-wordcloud" class="wordcloud animate__animated animate__fadeIn" style="animation-delay: 0.5s;">

                                        </div>
                                        
                                        <div class="users">
                                            <div class="emotions"> 
                                                <span style="font-size:11px; font-weight:normal; animation-delay: 0.6s;" class="animate__animated animate__fadeIn animate__faster"> 
                                                    Relevant users 
                                                </span>
                                                <span class="n-likes animate__animated animate__fadeIn animate__faster" style="font-size: 32px; animation-delay: 0.6s;">
                                                    1. ${topicMetadata.users[0]}
                                                </span>
                                                <span class="n-likes animate__animated animate__fadeIn animate__faster" style="font-size: 24px; animation-delay: 0.7s;">
                                                   2. ${topicMetadata.users[1]}
                                                </span>
                                                <span class="n-likes animate__animated animate__fadeIn animate__faster" style="font-size: 18px; animation-delay: 0.8s;">
                                                   3. ${topicMetadata.users[2]}
                                                </span>
                                                <span class="n-likes animate__animated animate__fadeIn animate__faster" style="font-size: 16px; animation-delay: 0.9s;">
                                                   4. ${topicMetadata.users[3]}
                                                </span>
                                                <span class="n-likes animate__animated animate__fadeIn animate__faster" style="font-size: 14px; animation-delay: 1.0s;">
                                                    5. ${topicMetadata.users[4]}
                                                </span>
                                            </div>
                                            <div class="sample-box animate__animated animate__fadeIn animate__faster"> 
                                                <button class="action-button">
                                                    ${"SEE MESSAGES"}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        `
                    )
                    $('.close-topic').click(() => {
                        $('.topic-detail').removeClass('animate__fadeIn');
                        $('.topic-detail').addClass('animate__fadeOut');
                        setTimeout(() => {
                            $('.topic-detail').remove();
                        }, 500);
                    });

                    $('.sample-box').click(() => {
                        $('.topic-container').append(
                            `
                            <div class="sample-detail animate__animated animate__fadeIn animate__faster">
                                <div class="close-sample">
                                    <span class="material-icons">
                                        highlight_off
                                    </span>
                                </div>
                                <div class="topic-name">
                                    <span style="font-size:11px; font-weight:normal;"> ${this.category} | Messages Sample </span>
                                    <div> ${this.series.name}</div>
                                </div>
                                    <div class="topic-content">
                                        <div class="topic-sentiments" style="width: 100%">
                                                ${topicMetadata.sample.slice(0, 5).map((message) => {
                                                    return `
                                                        <div class="likes animate__animated animate__fadeIn animate__faster" style="animation-delay: 0.1s;">  
                                                            <span class="n-likes" style="font-size:14px; margin-top:16px;">
                                                                ${message}
                                                            </span>
                                                        </div>
                                                    `;
                                                }).join(' ')}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            `
                        );
                        $('.close-sample').click(() => {
                            $('.sample-detail').removeClass('animate__fadeIn');
                            $('.sample-detail').addClass('animate__fadeOut');
                            setTimeout(() => {
                                $('.sample-detail').remove();
                            }, 500);
                        });
                    });

                    let wordcloudData = Object.entries(topicMetadata.keywords).map(([name, weight]) => { return {name, weight} })
                    console.log(wordcloudData);
                    // // build this array
                    // let wordcloudData = [
                    //     {name: "Trump", weight: 10}, 
                    //     {name: "America", weight: 8}, 
                    //     {name: "This", weight:  7},
                    //     {name:"That", weight: 5}, 
                    //     {name: "Other", weight: 4},
                    //     {name: "No", weight: 4}, 
                    //     {name: "Yes", weight: 3},
                    //     {name: "Anger", weight: 2}, 
                    //     {name: "Sadness", weight: 2}, 
                    //     {name: "Emotion", weight: 2}
                    // ]
                    
                    setTimeout(() => {
                        Highcharts.chart('topic-wordcloud', {
                            exporting: {
                                enabled: false
                            },
                            chart: {
                                backgroundColor: 'rgba(0,0,0,0)'
                            },
                            series: [{
                                type: 'wordcloud',
                                data: wordcloudData,
                                name: 'Occurrences'
                            }],
                            title: null
                        });
                    }, 500);
                }
            }
        }
      }
    },
    series: Object.values(topicsSeriesMap)
  });

  setTimeout(() => {
    $('.highcharts-xaxis-labels text').click(() => {
        console.log($(this).attr("y"));
    })
  }, 3000);